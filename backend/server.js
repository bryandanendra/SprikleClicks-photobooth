...
app.post("/generate-qr-code", async (req, res) => {
  ...
  try {
    ...
    const photoUrl = `https://picapica.onrender.com/photos/${photoId}`;
    ...
    console.log(`Photo URL: ${photoUrl}`); // Tambah log untuk debugging
...
// Add endpoint to serve the photos when scanned
app.get('/photos/:id', (req, res) => {
  const photoId = req.params.id;
  const photoPath = path.join(uploadDir, 'qr-photos', `${photoId}.jpg`);
  
  if (fs.existsSync(photoPath)) {
    // Serve the photo with a nice HTML wrapper
    ...
    const html = `
      ...
      <body>
        ...
        <footer>© 2025 Picapica. All Rights Reserved.</footer>
      </body>
      </html>
    `;
...
// Root endpoint
app.get("/", (req, res) => {
  res.send("Picapica Backend is ");
});

// Proxy endpoint untuk mengambil foto dari picapica
app.get('/view-photo/:id', async (req, res) => {
  const photoId = req.params.id;
  
  try {
    // Ambil konten dari picapica
    const response = await axios.get(`https://picapica.onrender.com/photos/${photoId}`);
    
    // Modifikasi konten HTML
    let html = response.data;
    
    // Ganti judul dan footer sesuai brand Anda
    html = html.replace('<title>Your Picapica Photo</title>', '<title>Your Picapica Photo</title>')
               .replace('Your Picapica Photo Strip!', 'Your Picapica Photo Strip!')
               .replace('© 2025 Ian Studio', '© 2025 Picapica')
               .replace('https://ianstudio.site', 'https://picapicabooth.com');
    
    res.setHeader('Content-Type', 'text/html');
    res.send(html);
    
  } catch (error) {
    // Jika foto tidak ditemukan atau error lainnya
    res.status(404).send(`
      <!DOCTYPE html>
      <html>
      <head>
