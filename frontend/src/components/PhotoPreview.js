...
const PhotoPreview = ({ capturedImages }) => {
  ...
  const generatePhotoStrip = useCallback(() => {
    ...
    const imgWidth = 400;  // Lebar gambar: 400 pixel
    const imgHeight = 300; // Tinggi gambar: 300 pixel
    const borderSize = 40;  // Ukuran border: 40 pixel
    const photoSpacing = 20;  // Jarak antar gambar: 20 pixel
    const textHeight = 50;   // Tinggi teks: 50 pixel
    const totalHeight = (imgHeight * 4) + (photoSpacing * 3) + (borderSize * 2) + textHeight;
    //img height untuk 4 gambar
    //photoSpacing untuk 4 foto butuh 3 spasi 
    //borderSize dikali 2 untuk kanan dan kiri
    //textHeight untuk teks
    ...
    const drawText = () => {
      ...
      ctx.fillText("UBAYA  " + timestamp, canvas.width / 2, totalHeight - borderSize * 1);
      ...
      if (selectedFrame === "mofusandImage") {
        ...
        drawMessBoothFrame(ctx, canvas, imgWidth, imgHeight, borderSize, photoSpacing);
      }
    };
