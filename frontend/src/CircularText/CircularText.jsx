...
const CircularText = ({
  ...
  const startRotation = useCallback((duration, rotation) => {
    controls.start({
      rotate: rotation + 360,
      scale: 1,
      transition: getTransition(duration, rotation),
    });
  }, [controls]);

  useEffect(() => {
    startRotation(spinDuration, currentRotation);
  }, [spinDuration, currentRotation, startRotation]);
